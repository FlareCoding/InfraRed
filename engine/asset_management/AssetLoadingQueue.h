...
namespace ifr
{
	...
	public:
		...
		void LoadAudioSourceAsset(const std::string& filepath, const std::string& name);
		...
		Ref<AudioSource> RetrieveAudioSourceAsset(const std::string& name);
	...
	private:
		...
		std::map<std::string, Ref<AudioSource>>		m_CustomLoadedAudioSources;
